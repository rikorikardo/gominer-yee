...
type HeaderReporter interface {
	...
	Submit(header []byte, job interface{}) (err error)
}

//HeaderProvider supplies headers for a miner to mine on
type HeaderProvider interface {
	//GetWork providers a header to mine on
	// the deprecationChannel is closed when the job should be abandoned
	GetWork() (target, header []byte, deprecationChannel chan bool, job interface{}, err error)
}
