...
func (m *Miner) createWork() {
	...
	for {
		...
		if isFirst == true || now-lastTime > 10 {
			...
			offset := 32 + 8 + 28
			if oldHeader[offset] < 0xFF {
				oldHeader[offset] = oldHeader[offset] + 1
			} else if oldHeader[offset+1] < 0xFF {
				oldHeader[offset+1] = oldHeader[offset+1] + 1
			} else {
				oldHeader[offset+2] = oldHeader[offset+2] + 1
			}
			extra := oldHeader[40:76]
...
func (miner *singleDeviceMiner) mine() {
	...
	for {
		...
		if nonceOut[0] != 0 || nonceOut[1] != 0 || nonceOut[2] != 0 || nonceOut[3] != 0 || nonceOut[4] != 0 || nonceOut[5] != 0 || nonceOut[6] != 0 || nonceOut[7] != 0 {
			log.Println(miner.MinerID, "-", "Nonce Founded")
			...
			go func() {
				if e := miner.Client.Submit(header, work.Job); e != nil {
